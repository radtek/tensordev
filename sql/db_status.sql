-- -----------------------------------------------------------------------------------
-- File Name    : db_status.sql
-- Description  : Displays information about database.
-- Requirements : Access to the DBA views.
-- Call Syntax  : @db_status.sql
...
col "TOTAL" format 999,999,999spool coleta_dados.txt
prompt 
prompt ===========================================================
...
select a.instance_name as "INSTANCIA", 
       ...
       b.platform_name as "SO", 
       a.status as "STATUS", 
       a.archiver as "MODO ARQUIVAMENTO",
...
select sum(bytes)/1024/1024 as "Tamanho (MB)" 
from dba_data_files;
...
select tablespace_name as "TABLESPACE", block_size as "BLOCO DE DADOS", status as "STATUS", logging as "LOGGING",extent_management as "GER.EXTENSAO", allocation_type as "TIPO DE ALOCACAO", segment_space_management as "GER.SEGMENTO",retention as "RETENCAO"
...
select decode(grouping(tablespace_name),0,null,1,'TOTAL (MB) =') as "1",tablespace_name as "TABLESPACE", 
       segment_type as "TIPO DE OBJETO", sum(bytes)/1024/1024 as "TAMANHO (MB)"
from dba_segments
...
select decode(to_char(last_analyzed,'DD-MM-RRRR'),null,'SEM ESTATISTICA',
       to_char(last_analyzed,'DD-MM-RRRR')) as "ANALISE",
